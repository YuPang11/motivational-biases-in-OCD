...
%   4) Shift ylabel("median estimate") slightly right
%      and downward by 1 mm
%% =========================================
close all; clc;
...
file_f2_csv = fullfile(dir_out, 'M21_f2_drawsocd.csv');
out_tiff    = fullfile(dir_out, 'M21_beta_distribution_OCD_70x26mm2.tif');
...
baseFont    = 9;
fontNameStr = "Arial";
xLabelStr   = [char(946), '_{ISPS-motor}'];

% OCD blue
faceCol  = [0.25 0.60 0.90];
lineCol  = [0 0 0];
lw       = 0.8;
...
% =============================
% Key change 1: set panel widths separately
% Keep the left panel unchanged; shorten the right panel
% =============================
plotW1 = (figW_cm - marginL - marginR - gap) / 2;   % Left panel width (original)
shrinkSubW_cm = 0.22;                               % Right panel reduction (recommended: 0.18~0.30)
plotW2 = plotW1 - shrinkSubW_cm;                    % Right panel width (shortened)
...
ax1 = axes('Parent', fig, 'Units', 'centimeters', ...
    'Position', [marginL, axY, plotW1, plotH]);
hold(ax1, "on");
...
% =========================================================
% Fix left-panel XTick to [0 0.05]
% =========================================================
fixedTicks = [0 0.05];

xl_now = xlim(ax1);
if xl_now(2) < fixedTicks(2)
    xlim(ax1, [xl_now(1), fixedTicks(2)]);
end

set(ax1, 'XTick', fixedTicks);
set(ax1, 'XTickLabel', {'0','0.05'});

%% =========================
% Panel 2: Right - subject level
% =========================
% Key change 2: keep the right panel right-aligned
leftPos2 = figW_cm - marginR - plotW2;

ax2 = axes('Parent', fig, 'Units', 'centimeters', ...
    'Position', [leftPos2, axY, plotW2, plotH]);
hold(ax2, "on");
...
% Keep the x-axis line, but remove ticks and tick labels
set(ax2, 'XTick', [], 'XTickLabel', []);
set(ax2, 'XColor', 'k');

% Key change 3: move "median estimate" downward by 1 mm
% and slightly to the right (scaled by plotW2)
dy_norm = 0.10 / plotH;    % 0.10 cm = 1 mm
dx_cm   = 0.10;            % Right shift by 1.0 mm (recommended: 0.08~0.15 cm)
dx_norm = dx_cm / plotW2;  % Note: use right-panel width plotW2
...
% Optional: print actual middle gap for fine-tuning
actualGap = leftPos2 - (marginL + plotW1);
fprintf("Actual middle gap = %.2f mm\n", actualGap*10);
