...
file_f2_csv = fullfile(dir_out, 'M16_f2_drawsocd.csv');
out_tiff    = fullfile(dir_out, 'M16_beta_distribution_OCD_70x35mm_Constrained2.tif');
...
xl1 = xlabel(ax1, xLabelStr, ...
    "FontName",fontNameStr, "FontSize", baseFont, "FontWeight","normal");
ylabel(ax1, "Density", ...
    "FontName",fontNameStr, "FontSize", baseFont, "FontWeight","normal");
...
% ===== White circle settings matching M12 =====
rng(1);
jitWidth   = 0.32;
markerSize = 8;
...
function beta = get_group_draws_from_f2(T, coln, beta_idx)
    ...
    if isempty(idx)
        ...
        hit = find(contains(cname,"beta") | contains(cname,"isps") | ...
                   contains(cname,"icpc") | contains(cname,"motor"), 1);
        if ~isempty(hit)
            idx = hit;
        end
    end
