function DrawOCD_WAIC_Set3right()
%% ==============================================================================
...
%   - Feature: Explicitly drawing M3c Baseline Marker (Thick line)
%
%  Requirements:
%   - Y axis on RIGHT
%   - X axis shown as 0 (right) -> negative (left)
%   - Reduce left whitespace, increase right whitespace (shift plot left)
%   - Margins: left_cm=0.45; right_cm=1.05; bottom_cm=1.35
% ==============================================================================
    clc; close all;

    % -----------------------------
    % 0) Global fonts: Arial 9pt, normal (no bold)
    % -----------------------------
    ...
    % ---------- PATHS (Set3 OCD) ----------
    waic_csv = 'F:\PIT\model\EEGmodelHC\HC\results\stan\plot\Set3_dataOCD.csv';
    ...
    if exist(waic_csv,'file')
        ...
        T_waic.Delta  = T_waic.WAIC - base_val;  % baseline -> 0
        T_waic.IsBest = (T_waic.WAIC == min(T_waic.WAIC));

        % ---------- 3) Display order (Set3) ----------
        ...
        % Keep original style: first appears at bottom
        ...
        % ---------- 6) X axis: 0 on right, negative to left ----------
        ...
        % IMPORTANT: XDir = normal, XLim = [negative 0]
        % => 0 will appear on the RIGHT, negatives on the LEFT
        set(ax1, 'XLim', [grid_low 0], 'XDir','normal');

        % ---------- 7) 0-line + baseline marker ----------
    ...
    outBase = fullfile(save_dir, 'OCD_Set3_WAIC_BaselineBold_right');
