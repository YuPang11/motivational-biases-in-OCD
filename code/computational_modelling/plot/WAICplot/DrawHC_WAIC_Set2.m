...
% ==============================================================================
    ...
    % ---------- PATHS ----------
    waic_csv = 'F:\PIT\model\EEGmodelHC\HC\results\stan\plot\Set2_data.csv';
    ...
    if exist(waic_csv,'file')
        ...
        % ---------- 1) Mapping (FIXED for Set2) ----------
        old_names = {'M9',  'M10', 'M15', 'M16'};
        new_names = {'M3c', 'M4a', 'M5a', 'M5b'};
        waic_map  = containers.Map(old_names, new_names);

        % robust: model column to cellstr
        ...
        % ---------- 2) Delta (baseline = raw M9 / displayed M3c) ----------
        base_model_raw = 'M9';
        ...
        % ---------- 3) Display order (FIXED for Set2) ----------
        ...
        % keep your original behavior (flip so first appears at bottom)
    ...
    outBase = fullfile(save_dir, 'HC_Set2_WAIC_BaselineBold');
