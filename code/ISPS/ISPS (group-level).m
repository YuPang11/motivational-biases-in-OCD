...
% load('H:\MT_all\final\correct_cue\cue_epoch\ft_lap\HC02.mat', 'freq'); % Use freq.elec/label only
load('F:\PIT\correct_all\OCD\tICPC_all.mat', 'tICPC','parTF');   % tICPC, parTF
load('H:\MT_all\final\correct_cue\cue_epoch\ft_lap\HC02.mat', 'freq'); % Use freq.elec/label only
...
csvfilename = fullfile(results_dir, 'ICPC_contra_ipsiocdplottest.csv');
...
%% ===== Export to CSV =====
dirs.home = 'H:\MT_all\cue\correct\HC';
results_dir = fullfile(dirs.home, 'results');
...
%% ===== Export to CSV =====
dirs.home = 'H:\MT_all\cue\correct\change6';
results_dir = fullfile(dirs.home, 'results');
if ~exist(results_dir, 'dir')
    mkdir(results_dir);
end
